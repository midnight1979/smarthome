...
#include <Ultrasonic.h>   // Для ультразвуковых датчиков HC-SR04

/* DS3231 */
DS3231 clock;
//RTCDateTime dt;

/* Указание портов для MAX6675 */
#define thermoDO 10
#define thermoCS 11
#define thermoCLK 12
...
// Инициализация OLED-дисплея (i2c)
...
long cm1 = 0;                               // Расстояние от ультразвукового датчика до поверхности воды в домашней емкости (в см.)
long cm2 = 0;                               // Расстояние от ультразвукового датчика до поверхности воды в уличной емкости (в см.)
...
//-------------------------------------------------------- Параметры емкостей ----------------------------------------------------------------------
//--------- Домашняя емкость --------
//int MainTank_Capacity     = 237;            // Объем домашней емкости в литрах
int MainTank_Height       = 112;            // Высота домашней емкости в см.
...
//-------- Уличная емкость ----------
//int StreetTank_Capacity       = 474;        // Объем домашней емкости в литрах
int StreetTank_Height         = 112;        // Высота домашней емкости в см.
...
void loop()
{
  ...
  // Проверка уровня воды в емкостях
  MainTankLevel();
  StreetTankLevel();
  HotWaterTankLevel();

  // Проверка температуры с термопары на MAX6675 (температура в баке с горячей водой) и с DS3231 (температура в доме)
  HotWaterTemp();
  HomeAirTemp();

  // Считываем параметры текущего времени и даты для вывода в SleepPage
...
// ----------------------------------------- Процедуры работы с термопарой MAX6675 -------------------------------
...
// END ------------------------------------- Процедуры работы с термопарой MAX6675 --------------------------- END
